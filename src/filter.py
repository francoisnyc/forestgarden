...
def match_agency(owner_name, agency_list):
    if not owner_name:
        return None
...
def run_buildability_tests(lot, config):
    reasons = []
...
def process_lots(geojson_path, deed_restrictions_path, config, db_conn):
...
def _borough_name_to_code(name):
